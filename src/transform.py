from collections import namedtuple # Import namedtuple to create named tuples, used to return query results
from enum import Enum # Import Enum to define a set of constant values representing query names
from typing import Callable, Dict, List # Import typing hints for functions and data structures
import pandas as pd # Import pandas for data manipulation
from pandas import DataFrame, read_sql # Import specific pandas structures and functions
from sqlalchemy import text # Import text to execute SQL queries from string
from sqlalchemy.engine.base import Engine # Import Engine to manage the database connection

from src.config import QUERIES_ROOT_PATH # Import the path where SQL queries are stored

# Create a named tuple to store the query name and its result
QueryResult = namedtuple("QueryResult", ["query", "result"])

# Create an enumeration class containing the name of each query to be executed
...
# Function to read an SQL file and convert it into an executable query
def read_query(query_name: str) -> str:
    ...
    with open(f"{QUERIES_ROOT_PATH}/{query_name}.sql", "r") as f:
        sql_file = f.read() # Read the entire file content as a string
        sql = text(sql_file) # Convert the string into an SQLAlchemy executable query
    return sql # Return the SQL query


def query_delivery_date_difference(database: Engine) -> QueryResult:
    # Function to get data about delivery date differences
    ...
    query = read_query(QueryEnum.DELIVERY_DATE_DIFFERENCE.value) # Read the SQL query from the file
    return QueryResult(query=query_name, result=read_sql(query, database)) # Execute the query and get the result as DataFrame
...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = pd.merge(items_products, orders, on="order_id")
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_ammount_per_date = filtered_dates["order_purchase_timestamp"].dt.date.value_counts().sort_index()
