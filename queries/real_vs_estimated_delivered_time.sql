...
-- Create a Common Table Expression (CTE) to filter only delivered orders
WITH delivered_orders AS (
    ...
    FROM olist_orders
    -- Only include orders that were delivered
      WHERE order_status = 'delivered' -- Ensure a valid delivery date exists
      AND order_delivered_customer_date IS NOT NULL
      AND order_estimated_delivery_date IS NOT NULL  
)
SELECT -- Main query starts here
    strftime('%m', order_purchase_timestamp) AS month_no, -- Extract the month number from the delivery date
    CASE strftime('%m', order_purchase_timestamp) -- Convert month number into abbreviated month name
        WHEN '01' THEN 'Jan'
        WHEN '02' THEN 'Feb'
        WHEN '03' THEN 'Mar'
        WHEN '04' THEN 'Apr'
        ...
        WHEN '12' THEN 'Dec'
    END AS month, 
    -- Calculate the average real delivery time for each year (delivery - purchase)
	...
	-- Calculate the average estimated delivery time for each year (estimated - purchase)
    AVG(CASE WHEN strftime('%Y', order_purchase_timestamp) = '2016'
...
-- Use the filtered delivered orders from the CTE
FROM delivered_orders
GROUP BY month_no-- Group by numeric and textual month to aggregate data per month
ORDER BY month_no; -- Sort result in calendar order by month number